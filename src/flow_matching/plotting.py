...
def evaluation_plots(losses, vector_field, path, device, 
                     num_samples, inf_params, N, modelparams, lambda_,
    ...
    if snapshots or make_corner:
       # generate one instance of simulated data to guide prior samples with
        ...
        x0 = path.p_simple.sample(num_samples).to(device) / lambda_
        ...
        if snapshots and N * len(inf_params) == 2:
            ...
            final_snapshot = solver.solve(x0, ts.view(1, nts, 1).expand(num_samples, nts, 1), y=simulations) * lambda_
